...
- `tiers`: the four levels of BabyRock Social, each with its HT monthly price, its TTC amount, its annual price and the country grid it belongs to. The build refuses a figure here that disagrees with `src/lib/skus.ts`, which is the only place a price is really written. `trials` carries the free test without a card and the one free month with a card.
